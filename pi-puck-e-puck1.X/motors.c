...
static uint16_t nbr_pas_left = 0;
static uint16_t nbr_pas_right = 0;
 
// interrupt for motor 1 (of two) = left motor
...
// interrupt for motor 2 (of two) = right motor
 ...
 * positive value to go forward and negative to go backward.
 */
void motors_set_speed_right(int motor_speed)
{
    if (motor_speed == 0) {
 ...
 * is running.
 */
uint16_t motors_get_steps_left(void)
{
    return nbr_pas_left;
}
 ...
 * is running.
 */
uint16_t motors_get_steps_right(void)
{
